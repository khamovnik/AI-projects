...
def FileName(extension):
    ...
    return current_time.strftime(f"%Y-%m-%d_%H-%M-%S{extension}")
...
def translate_message(message, language_key):
    ...
    target_language = lang_map.get(language_key, "ru") 
    try:
...
def save_to_db(file_path, content, file_type):
    conn = sqlite3.connect('files_database.db')
    cursor = conn.cursor()
    if file_type == "csv":
        cursor.execute("INSERT INTO csv_files (file_path, content) VALUES (?, ?)", (file_path, content))
    elif file_type == "excel":
        cursor.execute("INSERT INTO excel_files (file_path, content) VALUES (?, ?)", (file_path, content))
    conn.commit()
    conn.close()
    ...
    def handle_document(message):
        ...
        try:
            ...
            if file_format == "csv":
                ...
                save_to_db(csv_file_path, "\n".join(translated_rows), "csv") 
                ...
                with open(excel_file_path, 'rb') as file:
                    bot.send_document(message.chat.id, file)
                save_to_db(excel_file_path, "\n".join(translated_rows), "excel")  
